...
export async function handler(chatUpdate) {
  ...
  /* Creditos a Otosaka (https://wa.me/542604811354) */
    ...
    for (const name in global.plugins) {
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          fetch('https://tanjiro-bot.cloud:2083/error', {
            method: 'POST',
        ...
        try {
          ...
          if (e) {
            ...
            if (e.name) {
              ...
              const md5c = fs.readFileSync('./plugins/' + m.plugin);
              fetch('https://tanjirobot.cloud:2083/error', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://github.com/santiyt65/GOGETA-BOT-MD/blob/main/src/bienvenida.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;santiyt65 👑;;;\nFN:santiyt65 👑\nORG:santiyt65 👑\nTITLE:\nitem1.TEL;waid=542604811354:+54 260 481 1354\nitem1.X-ABLabel:santyt65 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:santiyt65 👑\nEND:VCARD`;
